import { player, background, intro, cano, chao, Over, Score, audios, scale } from "./itens.js";
    ...
    if(gameOn === 1){
        ...
        for(let i = 0; i < cano.positions.length; i++){
            ...
            if(gameOver === 0){
                ...
                if(p.x < -300*cano.scale){
                    cano.positions.shift()
                }
                
                if(p.x+cano.tamanhoX*cano.scale < 0 && p.x+cano.tamanhoX*cano.scale > -5){
                    count += 1
                    localStorage.setItem('Score',count)
                    audios.Score_Sound.play()
                    if(parseInt(localStorage.getItem('Score')) > parseInt(localStorage.getItem('Best')) && parseInt(localStorage.getItem('Best')) !== 0 && controle === 0){
        ...
        if(player.positionY + player.tamanhoY*scale/2 <= canvas.height - chao.tamanhoY*scale && gameOver === 0){
...
function jump(){
    player.velocityY = -6*scale
    tempo = 2
}
function reset(){
    player.positionY = 125*scale
